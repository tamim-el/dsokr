...
prefix_file_name_w_dir = 'toy_DOKR/' + prefix_file_name
...
with open(prefix_file_name_w_dir + 'lev_scores.npy', 'wb') as f:
    np.save(f, lev)
...
plt.tight_layout()
plt.savefig('Plots/toy_lev_scores.pdf', transparent=True)
plt.close()
...
def train_normal(epoch, net, verbose=True, save=True):
...
def validate_normal(net, verbose=True, save=True):
...
def train(epoch, net, verbose=True, save=True):
...
def validate(net, verbose=True, save=True):
    ...
    if save:
        with open(prefix_file_name_w_dir + 'mse_validations.npy', 'wb') as f:
            np.save(f, mse_validations)
...
for i_osn, output_sketch_name in enumerate(output_sketch_names):
    ...
    prefix_file_name_w_dir_perfecth = 'toy_DOKR/' + prefix_file_name_perfecth
...
plt.tight_layout()
plt.savefig('Plots/toy_perfecth.pdf', transparent=True)
plt.close()
...
ax.set_yticklabels([str(i) for i in ax.get_yticks()], fontsize = 50)
plt.tight_layout()
plt.savefig('Plots/toy_mse.pdf', transparent=True)
plt.close()